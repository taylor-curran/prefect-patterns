...
def dbt_flow(sim_failure: SimulatedFailure = default_simulated_failure):
    run_result_a = trigger_dbt_cloud_job_run_and_wait(
        dbt_cloud_credentials=DbtCloudCredentials.load("my-dbt-creds"),
        job_id=179098)
    h = upstream_task_h()
    i = upstream_task_i()
